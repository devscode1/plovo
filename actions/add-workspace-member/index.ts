  ...
  try {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        return { error: "User not found. They must sign up to Plovo first before you can invite them." };
      }
      throw error;
    }
    ...
    try {
      await sendEmail({
        ...
        html: `<p>Hello ${targetUser.displayName || targetUser.email},</p><p>You have been added as a <strong>Member</strong> to a Plovo workspace. You can now view boards and edit your assigned tasks.</p>`,
      });
    } catch (err) {
    ...
    console.error("addWorkspaceMember action error:", error);
    return { error: `Failed to add member: ${error instanceof Error ? error.message : String(error)}` };
  }
};
